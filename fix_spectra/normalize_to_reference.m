...
%
% This is useful if there is an internal standard etc. Usually deconvolves
% the reference area but in the special case where there are no
% non-reference peaks and one of the reference peaks is located at 0.0,
% just sums the area from left to right. (The reason it doesn't do this
% everywhere is that other areas may not have internal non-reference peaks,
% but may have significant contributions from external peaks. These
% contributions need to be removed by the deconvolution.)
%
% -------------------------------------------------------------------------
% Authors
% -------------------------------------------------------------------------
...
%
% Eric Moyer (May 2012, Dec 2012) eric_moyer@yahoo.com
%
...
sum_instead_of_deconvolve = any(reference_peaks == 0.0) & isempty(non_reference_peaks);
...
for c = 1:length(collections)
    ...
    for s = 1:collections{c}.num_samples
        % Calculate the area under the reference peaks
        if sum_instead_of_deconvolve
            y_val = collections{c}.Y(fit_inxs, s);
            sum_all_reference_peaks = sum(y_val);
        else
        ...
        % Deal with display variables if did a deconvolution
        if ~sum_instead_of_deconvolve
            try
