...
% Last Modified by GUIDE v2.5 03-Jan-2010 23:40:32
...
% Updates the count, so it can be used as error check.
handles.tableCountTracker = handles.tableCountTracker + 1;
...
if(handles.tableCountTracker < 1)
    % Nothing to remove, ignoring request.
else
    counter = 1;
...
% Creates a copy of the subSetMolecules saves it based on the user's name
% given in the popup window.  
...
%show_molecule(handles.subSetMolecules(row));
