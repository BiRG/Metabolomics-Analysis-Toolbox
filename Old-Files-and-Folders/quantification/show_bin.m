...
if (~exist('hide_plot') || ~hide_plot) && isfield(info,'active_adj') && info.active_adj
    ...
    for i = 1:length(collections)
        for j = 1:collections{i}.num_samples
            ...
            line(adj_saved_data{i}.x{j},adj_saved_data{i}.Y_residual{j},'Color',[0.5,0.5,0.5]);
            myfunc = @(hObject, eventdata, handles) (plot_line_adjacent(main_h,i,j,left,right,left_handle));
            set(hl,'ButtonDownFcn',myfunc);
            if j == 1
                yhs(end+1) = hl;
                % Change the following line if you want to change the legend for
        ...
        for j = 1:collections{i}.num_samples
            ...
            for m = 1:nm(2)
                if sum(match_inxs(:,m) > 0) == nm(1) % Match across all samples
                    ...
                    text(adj_saved_data{i}.bin_X{j}(match_inxs(cnt,m)),adj_saved_data{i}.Y_residual{j}(ix)+adj_saved_data{i}.Y_fit{j}(ix),num2str(mcnt));
                    mcnt = mcnt + 1;
                end
            end
            cnt = cnt + 1;
