function line_click_info_key_press(c,s,main_h)
...
elseif strcmp(k,'leftarrow')
    ...
    set(gca,'xlim',[xlim1(1)+xdist,xlim1(2)+xdist]);
elseif strcmp(k,'j') % Previous spectrum    
    s = s - 1;
    if s <= 0
        c = c - 1;
        if c >= 1
    ...
    plot_func = getappdata(gcf,'plot_func');
    plot_func(c,s);
elseif strcmp(k,'k') % Next spectrum
    s = s + 1;
    if s > collections{c}.num_samples
        c = c + 1;
        s = 1;
        if c > length(collections)
            return
        end
    end
    plot_func = getappdata(gcf,'plot_func');
