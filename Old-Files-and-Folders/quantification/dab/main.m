...
bin_str = contents{get(hObject,'Value')};
if strcmp(bin_str,'');
    xlim auto;
    ylim auto;
    return;
...
if bin(1) < bin(2)
    inverted_bin = true;
    handles.xlim = [bin(1),bin(2)];
else
    handles.xlim = [bin(2),bin(1)];
end
...
% hObject    handle to update_selected_bin_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
left_cursor = findobj(gcf,'tag','left_cursor');
right_cursor = findobj(gcf,'tag','right_cursor');
if isempty(left_cursor) || isempty(right_cursor)
    return;
end
left = get_cursor_location(left_cursor);
right = get_cursor_location(right_cursor);
bins(bin_inx,:) = [left,right];
