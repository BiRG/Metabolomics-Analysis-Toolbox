function converted = convert_to_old_format(collection)
    ...
    for i = 1:size(keys,1)
        ...
        if min(field_size) == 1 % we have a single column 'label'
            if max(field_size) == wavenumber_count % we have row labels
                if field_size(1) ~= wavenumber_count % ensure number of rows agree
                    collection.(key) = collection.(key)';
                end
            elseif max(field_size) == sample_count % we have row labels
                if field_size(2) ~= sample_count % ensure number of columns agree
                    collection.(key) = collection.(key)';
                end
            end
        elseif min(field_size) == sample_count % this is most likely y
            if field_size(2) ~= sample_count % ensure that the number of rows is sample count
                collection.(key) = collection.(key)';
            end
        end
    end
