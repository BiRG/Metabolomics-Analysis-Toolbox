...
system('mkdir',catdir($src_name,'.git'));
...
system('mkdir',catdir($src_name,'pubd_full_1'));
...
system('mkdir',catdir($src_name,"pubd_empty"));
system('mkdir',catdir($src_name,'privd_full_1'));
...
system('mkdir',catdir($src_name,"privd_empty"));


#Changes for this particular test
system('mkdir','-p',catdir($src_name,'public_only','renamed_pub_only_dir'));
system('echo "Txt: pub_only_file" > '.
       catfile($src_name,'public_only','renamed_pub_only_file'));
system('echo "Txt: pub_only_sub_file" > '.
       catfile($src_name,'public_only','renamed_pub_only_dir','pub_only_sub_file'));
...
{ 
    ...
    public_only=>{
	renamed_pub_only_file=>"Txt: pub_only_file\n",
	renamed_pub_only_dir=>{
	    pub_only_sub_file=>"Txt: pub_only_sub_file\n"
	},
    },
    privd_empty=>{},
};
...
# + /pubd_empty
# 
system('mkdir',catdir($dest_name,'.git'));
...
system('mkdir',catdir($dest_name,'pubd_full_1'));
system('echo "Txt: pubd_full_1_file_1" > '.
       catfile($dest_name,'pubd_full_1','pubd_full_1_file_1'));
system('echo "Txt: pubd_full_1_file_2" > '.
       catfile($dest_name,'pubd_full_1','pubd_full_1_file_2'));
system('mkdir',catdir($dest_name,"pubd_empty"));


#Changes for this test only

system('mkdir','-p',catdir($dest_name,'pub_only_dir'));
system('echo "Txt: pub_only_file" > '.
       catfile($dest_name,'pub_only_file'));
system('echo "Txt: pub_only_sub_file" > '.
       catfile($dest_name,'pub_only_dir','pub_only_sub_file'));
...
{ 
    ...
    pub_only_file=>"Txt: pub_only_file\n",
    pub_only_dir=>{
	pub_only_sub_file=>"Txt: pub_only_sub_file\n"
    },
    pubd_empty=>{}
