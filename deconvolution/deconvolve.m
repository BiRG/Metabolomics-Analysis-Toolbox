function results = deconvolve(x,y,maxs,mins,options)
% x (d x 1) and y (d x 1)
if ~exist('options')
    options = {};
...
regions = determine_regions(x,y,maxs,mins,options.min_width,options.max_width);
...
function regions = fit_regions(regions)
% Fit each region
for r = 1:length(regions)
    if regions{r}.num_maxima > 0
...
function regions = determine_regions(x,y,maxs,all_mins,min_width,max_width)
% Divide the problem up into regions
biggest_gap = max(abs(all_mins(:,1)-all_mins(:,2)));
mins = all_mins(:,1);
i = 1;
regions = {};
r = 1;
while (i <= length(x))
    ...
    regions{r}.lb = lb;
    regions{r}.ub = ub;
    regions{r}.inxs = sub_inxs;
    regions{r}.num_maxima = length(BETA0)/4;
