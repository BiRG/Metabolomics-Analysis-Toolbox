    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying standalone applications, it is possible 
              to run the shell script file run_Adjacent_Deconvolution_v1_beta.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    NOTE: Adjacent_Deconvolution_v1_beta.ctf will be present only if you have chosen 
    ...
    All Platforms:
        * Adjacent_Deconvolution_v1_beta.ctf (if present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * Adjacent_Deconvolution_v1_beta.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * Adjacent_Deconvolution_v1_beta (application created by MATLAB Compiler)
        * run_Adjacent_Deconvolution_v1_beta.sh (shell script that can be run to 
    ...
    All Platforms:
        ...
        * Adjacent_Deconvolution_v1_beta.dll (shared library, file extension varies by 
          platform)
        * Adjacent_Deconvolution_v1_beta.h (library header file)
    ...
    Windows:
        * Adjacent_Deconvolution_v1_beta.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)    
        * Adjacent_Deconvolution_v1_beta.xml (documentation files)
        * Adjacent_Deconvolution_v1_beta.pdb (program database file - if DEBUG option 
          selected)
        * Adjacent_Deconvolution_v1_beta.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    Windows:
        * Adjacent_Deconvolution_v1_beta.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * Adjacent_Deconvolution_v1_beta_1_0.dll (COM component)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)      
    ...
    All Platforms: 
        * Adjacent_Deconvolution_v1_beta.jar (Java class files and compiled M-code)
    ...
    Windows:
        * Adjacent_Deconvolution_v1_beta.ctf (if present)(Component Technology File archive, contains 
          compiled M-code)
        * Adjacent_Deconvolution_v1_beta_1_0.dll (COM component)
        * Adjacent_Deconvolution_v1_beta.xla (optionally created and placed in
          C:\Users\Paul\BiRG\Research\Toxicology\omics_analysis\deconvolution\Adjacent_Deconvolution_v1_beta\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
