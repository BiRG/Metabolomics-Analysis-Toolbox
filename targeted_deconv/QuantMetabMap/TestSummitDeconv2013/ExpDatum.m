...
% 
    ...
    methods(Static)
        ...
        % Return a ExpDatum with the properties set to the values passed in.
        %
        ...
        % Example:
        %
        % >> g = ExpDatum.dangerous_constructor([],2,3,4,5,6,7,'my id')
        %
        % Produces a completely unusable ExpDatum object
            obj = ExpDatum;
    ...
    methods
        function obj=ExpDatum(spectrum_width)
        % Generate a random data point for the given spectrum width
        %
        % Usage: ExpDatum(spectrum_width)
        %
        ...
        % Generates and deconvolves peaks in a spectrum 0.5 ppm wide
            if nargin > 0
                ...
                deconvolvers = ...
                    ExpDeconv.deconvolution_starting_point_method_names;
                ...
                for p = 1:length(pickers)
                    for d = 1:length(deconvolvers)
                        tmp = ExpDeconv(obj.id, obj.spectrum, ...
        ...
        % ----------------------------------------------------------------
        %
        % obj - a single ExpDatum object. Must have id,
        ...
        % matrix of objects.
            ...
            separator = '';
            for i = 1:length(objs)
                strs{i} =sprintf([separator,'ExpDatum(%g, %s)'], objs(i).spectrum_width, objs(i).id);
                separator = '\n';
            end
            str = sprintf('%s',strs{:});
        end
